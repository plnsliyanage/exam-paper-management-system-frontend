  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await lecturerApi.addMarkingScripts({
        packetId: packet.id,
        scriptsCount: Number(scriptCount)
      });
    ...
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-md overflow-hidden">
        ...
        <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
          <div>
            <h2 className="font-bold text-slate-900 text-sm">Log Marking Progress</h2>
            <p className="text-xs text-slate-500">{packet.courseCode} - {packet.courseName}</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 text-xs">
          <div>
            <label className="font-bold text-slate-700 block mb-1">Number of Answer Scripts Assigned/Marked</label>
            <input
              type="number"
              min="0"
              required
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            >
              <CheckCircle className="w-4 h-4" /> {loading ? 'Saving...' : 'Save Scripts'}
            </button>
          </div>
        </form>
