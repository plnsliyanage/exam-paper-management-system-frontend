...
export default function HodDepartmentView({ deptId = "ALL" }) {
  ...
  const printingCount = stats?.printingPackets ?? packets.filter((p) => (p.status || "").toUpperCase() === "PRINTING").length;
  const completedCount = stats?.completedPackets ?? packets.filter((p) => (p.status || "").toUpperCase() === "COMPLETED").length;
  ...
  const summaryCards = useMemo(() => {
    return [
      ...
      },
      {
        ...
        desc: "Fully finalized",
      },
    ];
  }, [totalPacketsCount, pendingCount, draftCount, submittedCount, approvedCount, printingCount, completedCount]);
